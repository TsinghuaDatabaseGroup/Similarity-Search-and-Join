...
void perform_join(int range_bound)
{
	...
	for (int k = range_bound; k < (int)records.size(); k++)
	{
		...
		for (int count = 0; count < qgram_prefix_length; count++)
		{
			...
			auto iter = index.find(qgram.first);
			...
			for (int t = item.pos; t < (int)list.size(); t++)
			{
				...
				if (cand != k && abs(list[t].second - qgram.second) <= THRESHOLD)
				{
		...
		for (int count = 0; count < qchunk_prefix_length; count++)
		{
			...
			index[qchunk.first].ids.emplace_back(k, qchunk.second);
		}

		for (int cand : occurances)
		{
			cand_num++;
			int real_overlap = overlap(cand, k);
			if (real_overlap < ((int)records[cand].length() + Q - 1) / Q - THRESHOLD) continue;
