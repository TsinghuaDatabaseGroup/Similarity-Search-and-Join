...
public class EditDistance {
	...
	public static boolean passContentFilter(byte[] xgroup, byte[] ygroup, int xlen, int ylen) {
		if (Config.ED_THRESHOLD <= 4) {
			int xscore = 0;
			int yscore = 0;
			for (int i = 0; i < Config.NUM_OF_GROUP; i++) {
				int abs = xgroup[i] - ygroup[i];
				if (abs > 0) {
					xscore += abs;
					if (xscore > Config.ED_THRESHOLD) return false;
				} else if (abs < 0) {
					yscore += (0 - abs);
					if (yscore > Config.ED_THRESHOLD) return false;
				}
			}
			if (xscore + yscore > 2 * Config.ED_THRESHOLD - Math.abs(xlen - ylen))
				return false;
			return true;			
		}
		int xscore = 0;
		int yscore = 0;
		int deltax = ylen - xlen;
		int deltay = 0;
		if (deltax < 0) {
			deltay = 0 - deltax;
			deltax = 0;
		}
		int xlim = Config.ED_THRESHOLD - deltax;
		int ylim = Config.ED_THRESHOLD - deltay;
		for (int i = 0; i < Config.NUM_OF_GROUP; i++) {
			int abs = xgroup[i] - ygroup[i];
			if (abs > 0) {
				xscore += abs;
				if (xscore > xlim) return false;
			} else if (abs < 0) {
				yscore += (0 - abs);
				if (yscore > ylim) return false;
			}
		}
		return true;
	}
