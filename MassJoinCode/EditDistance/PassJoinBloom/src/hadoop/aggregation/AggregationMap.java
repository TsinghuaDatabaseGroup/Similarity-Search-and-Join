...
public class AggregationMap extends Mapper<Text, Text, VIntWritable, IntWritable>{
	...
	public void map(Text recordID, Text record, Context context) throws IOException, InterruptedException{
		...
		if (path.contains(Config.RECORD1)) {
		    ...
		    int[] segments = GetSeg.getSeg(length, Config.NUM_OF_SEGMENT);
	    	for (int segID = 0; segID < Config.NUM_OF_SEGMENT; segID++) {
	    		int start = segments[2 * segID];
	    		int segLen = segments[2 * segID + 1];
				int subToken = GetToken.getToken(data, start, segLen);
				int code = Hash.getHashCode(subToken, segID);
				int segment = Math.abs(code) % Config.EXCEPT_NUM_OF_ELEMENT;
				int thekey = segment % Config.NUM_OF_BITSET;
				context.write(new VIntWritable(thekey), new IntWritable(segment / Config.NUM_OF_BITSET));
			}
